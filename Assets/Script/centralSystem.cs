...
public class centralSystem : MonoBehaviour {
	...
	//配信用の入出力管理スクリプト
	private TypingSystemGeneralization typingSystemGeneralization;
	...
	void Start() {
		...
		typingSystemGeneralization = GetComponent<TypingSystemGeneralization>();
	...
	private void SendText(string text) {
		if (typingSystemGeneralization != null) {
			typingSystemGeneralization.listenKeyEvent(text);
		} else if (typingSystem == null) {
			//
		} else {
			//実験用
